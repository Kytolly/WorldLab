...
统一 WorldModelStepResult 的 action、frames、state、reward 输出校验。完善 WorldModelSimulator 的 chunk-level reset/step 数据流。建立适配ExampleWorldModel最小 ExampleTask 与 ExampleEnvironment，明确任务、环境和模型的边界。

在 Dashboard 展示实时 synthetic frames、动作序列、奖励、状态和事件时间线。验证运行时能看到帧、动作、reward、chunk 索引和模型耗时；浏览器不需要读取内部 Python 对象。
