...
from gesim.models.gesim_v2.video_processor import VideoProcessor
from gesim.models.gesim_v2.networks import AutoencoderKLWan, CosmosTransformer3DModel
from gesim.models.gesim_v2.schedulers.scheduling_flow_match_euler_discrete import FlowMatchEulerDiscreteScheduler
...
class Cosmos2VideoToWorldPipeline(DiffusionPipeline):
    ...
    def _get_t5_prompt_embeds(
        ...
        if is_main:
            for length in lengths.tolist():
                pad_len = max_sequence_length - int(length)
                logger.debug("text_len=%d pad_len=%d", int(length), pad_len)
    ...
    @classmethod
    def from_pretrained(cls, model_id, torch_dtype=torch.bfloat16, device='cuda'):
