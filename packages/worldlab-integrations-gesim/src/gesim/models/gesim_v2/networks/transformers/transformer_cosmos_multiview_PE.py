    ...
    def forward(
        ...
        if return_pose:
            ...
            pose_temb, pose_embedded_timestep = self.pose_time_embed(
                ...
                # diffusers' AdaLayerNormSingle expands this mapping with
                # ``**added_cond_kwargs`` even when extra conditions are off.
                # Its PixArt embedding also requires these unused positional
                # inputs in newer diffusers releases.
                added_cond_kwargs={
                    "resolution": torch.zeros(
                        (pose_batch_size, 2), device=pose_timestep.device, dtype=pose_timestep.dtype
                    ),
                    "aspect_ratio": torch.zeros(
                        (pose_batch_size, 1), device=pose_timestep.device, dtype=pose_timestep.dtype
                    ),
                },
            )
